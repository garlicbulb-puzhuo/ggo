import os, dicom, fnmatch, re, os.path
from dicom.filereader import InvalidDicomError

DIR = './CA1'
img_dir = DIR+'/DICOMDAT'
mask_dir = DIR+'_MASK'
img_list = [os.path.join(dirpath,f) for dirpath, dirnames, files in os.walk(img_dir) for f in fnmatch.filter(files, "IMG*")]
img_dict = {}
for img in img_list: 
	try:		
	...
	if seriesNumber not in img_dict[patientID]['img_series']: 
		...
		img_dict[patientID]['img_series'][seriesNumber]['path'] = re.search(r'(.*SRS\d{1,})(/IMG\d{1,})',img).group(1)
	...
	path = mask_dir+'/'+re.search(r'(.*DICOMDAT/)(SDY.*)',img_dict[patientID]['img_series'][seriesNumber]['path']).group(2)
	mask_name = re.search(r'(.*SRS\d{1,}/)(IMG\d{1,})',img).group(2) + '-mask.tif'
	mask_path = os.path.join(path,mask_name)
			#print mask_path
	if os.path.isfile(mask_path): 
		img_dict[patientID]['img_series'][seriesNumber]['masks'].append(mask_path)
		img_dict[patientID]['img_series'][seriesNumber]['mask_number'] += 1
	else: 
		img_dict[patientID]['img_series'][seriesNumber]['masks'].append(None)
