...
do
    ...
    if [ ${processor} == "preprocessor" ]; then
        echo python -m scripts.preprocess.watershed --input_dir ${input_batch_dir} --output_path ${output_dir}/${batch}
        nohup python -m scripts.preprocess.watershed --input_dir ${input_batch_dir} --output_path ${output_dir}/${batch} > ${log_dir}/${batch}/job.log 2>&1 &
